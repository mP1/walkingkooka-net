...
/**
 * A {@link HeaderParser} that rebuilds the {@link String} with everything but comments.
 */
final class CommentRemovingHeaderParser extends HeaderParser {
    ...
    private static String removeCommentsNonEmptyText(final String text) {
        final CommentRemovingHeaderParser parser = new CommentRemovingHeaderParser(text);
        parser.parse();
        return parser.without.toString();
    }

    private CommentRemovingHeaderParser(final String text) {
        super(text);
    }
