 ...
 * </pre>
 */
final class EncodingHeaderParser extends HeaderParser {

    static Encoding parseEncoding(final String text) {
        final EncodingHeaderParser parser = new EncodingHeaderParser(text);
        parser.parse();
        return parser.encoding;
    }

    private EncodingHeaderParser(final String text) {
        super(text);
    }
    ...
    @Override
    void token() {
        this.encoding = this.token(EncodingHeaderParser.RFC2045TOKEN, Encoding::with);
    }
