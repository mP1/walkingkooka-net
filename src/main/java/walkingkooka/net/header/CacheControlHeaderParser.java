 ...
 * </pre>
 */
final class CacheControlHeaderParser extends HeaderParser {

    static CacheControl parseCacheControl(final String text) {
        final CacheControlHeaderParser parser = new CacheControlHeaderParser(text);
        parser.parse();
        return CacheControl.with(parser.directives);
    }

    private CacheControlHeaderParser(String text) {
        super(text);
    }
