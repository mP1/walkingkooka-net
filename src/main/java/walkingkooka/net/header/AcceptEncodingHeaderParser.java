 ...
 * </pre>
 */
final class AcceptEncodingHeaderParser extends HeaderParserWithParameters<AcceptEncodingValue, AcceptEncodingValueParameterName<?>> {

    static AcceptEncoding parseAcceptEncoding(final String text) {
        final AcceptEncodingHeaderParser parser = new AcceptEncodingHeaderParser(text);
        parser.parse();
        parser.encodings.sort(HasQualityFactor.qualityFactorDescendingComparator());
        return AcceptEncoding.with(parser.encodings);
    }

    private AcceptEncodingHeaderParser(final String text) {
        super(text);
    }
