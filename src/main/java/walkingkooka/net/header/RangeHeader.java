...
public final class RangeHeader implements Header,
    ...
    public static RangeHeader parse(final String header) {
        ...
        if (header.isEmpty()) {
            throw new HeaderException("Header text empty");
        }

        final int equalsAfterUnit = header.indexOf('=');
        if (-1 == equalsAfterUnit) {
            throw new HeaderException("Missing unit and '=' from header value " + CharSequences.quote(header));
        }
    ...
    private static Range<Long> range(final String value) {
        ...
        if (0 == dash) {
            throw new HeaderException("Header range missing start " + CharSequences.quote(value));
        }
    ...
    /**
     * Factory that creates a new {@link RangeHeader}
     */
    public static RangeHeader with(final RangeHeaderUnit unit, final List<Range<Long>> ranges) {
        checkUnit(unit);

        return new RangeHeader(unit, copyAndCheckValue(ranges));
    }

    /**
     * Private ctor use factory.
     */
    private RangeHeader(final RangeHeaderUnit unit, final List<Range<Long>> ranges) {
        super();
        this.unit = unit;
        this.ranges = ranges;
    }
     ...
     * {@link HeaderException} will be thrown.
     */
    private static List<Range<Long>> copyAndCheckValue(final List<Range<Long>> ranges) {
        ...
        if (0 == count) {
            throw new HeaderException("Missing range");
        }
