...
public abstract class HeaderHandlerTestCase<H extends HeaderHandler<T>, T> implements PackagePrivateClassTesting<H>,
    ParseStringTesting<T>,
    ToStringTesting<H>,
    TypeNameTesting<H> {

    HeaderHandlerTestCase() {
        super();
    }
    ...
    final void toTextAndCheck(final H handler, final T value, final String expected) {
        this.toTextAndCheck(handler, value, this.name(), expected);
    }

    final void toTextAndCheck(final H handler, final T value, final Name name, final String expected) {
